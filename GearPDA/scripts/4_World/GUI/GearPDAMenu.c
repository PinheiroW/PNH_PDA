class GearPDAMenu extends UIScriptedMenu
{
	...
	void GearPDAMenu()
	{				
		if (GearPDADebugMode) Print(GearPDAModPreffix + "GearPDAMenu construct");
	}	
	
	void ~GearPDAMenu()
	{
		if (GearPDADebugMode) Print(GearPDAModPreffix + "GearPDAMenu destruct");
    ...
    {
		if (GearPDADebugMode) Print(GearPDAModPreffix + "GearPDAMenu init");
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "GearPDA/scripts/layouts/GearPDAMenu.layout" );
	...
	{
		if (GearPDADebugMode) Print(GearPDAModPreffix + "FillContactsList");
        ...
        if (pluginGearPDA.m_enableGlobalChat)
		{
			...
			m_contactList.SetItem(itemId, "" + pluginGearPDA.m_globalChatUnreaded, NULL, 1);
		...
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation: X1 " + selectedRow);
		...
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation: X2 " + 0);
	...
	{
		if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation: " + id);
		...
		if (pluginGearPDA.m_enableGlobalChat)
		{
			if (id == rowShift)
			{
				...
				pluginGearPDA.m_globalChatUnreaded = 0;
		...
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "No need to select conversation: " + id);
			m_chat.ClearItems();
			m_message.Enable(false);
			m_send.Enable(false);
			m_sendFuncEnabled = false;
	...
	override void Update(float timeslice)
	{
		...
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation: X5");
			FillContactsList();
		...
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation: X3 " + m_lastSelectedContact);
			FillContactsList();
		...
		if (m_dirty)
		{
			if (GearPDADebugMode) Print(GearPDAModPreffix + "SelectConversation by dirty: X6");
	...
	override bool OnKeyPress(Widget w, int x, int y, int key) {
		if (GearPDADebugMode) Print(GearPDAModPreffix + "OnKeyPress: " + w.GetName());
			...
			if (selectedRow >= 0 && message.LengthUtf8() > 0)
			{
				...
				if (pluginGearPDA.m_enableGlobalChat)
				{
					if (selectedRow == rowShift)
					{
						...
						GetRPCManager().SendRPC( GearPDAModPreffix, "SendGlobalMessage", new Param1<string>( message ), true );
						m_message.SetText("");	
						return true;				
					}
				...
				{					
					...
					m_sendMessageStatus = 1;
					GetRPCManager().SendRPC( GearPDAModPreffix, "SendMessage", new Param2<string, string>( target, message ), true );
					m_message.SetText("");
					return true;
				}
			}
	...
	override bool OnClick(Widget w, int x, int y, int button) {
		...
		if (GearPDADebugMode) Print(GearPDAModPreffix + "OnClick: " + w.GetName());
			...
			if (w == m_addContactBtn)
			{
				if (m_addContactTimeout <= 0 && m_addContactStatus == 0)
				{
					...
					if (contactId.LengthUtf8() <= m_contactMaxLength && contactId.LengthUtf8() > 0) {
						...
						GetRPCManager().SendRPC( GearPDAModPreffix, "AddContact", new Param1<string>( contactId ), true );
						return true;
					}	
				}
			}
	...
	override bool OnChange(Widget w, int x, int y, bool finished) {
		...
		if (GearPDADebugMode) Print(GearPDAModPreffix + "OnChange: " + w.GetName());
	...
	override bool OnItemSelected(Widget w, int x, int y, int row, int  column,	int  oldRow, int  oldColumn)
	{
		...
		if (GearPDADebugMode) Print(GearPDAModPreffix + "OnItemSelected: " + w.GetName());
